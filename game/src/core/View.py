...
class View:
    def __init__(self):
        ...
        self.__viewSize = Window.WIDTH // Sprite.GROUND_TILE_SIZE + 2 * Display.BUFFER, Window.HEIGHT // Sprite.GROUND_TILE_SIZE + 2 * Display.BUFFER
        print(self.__viewSize)
        self.__view = []
    ...
    def setView(self, map, player):
        playerRow = player.getIndex()[0]
        playerCol = player.getIndex()[1]
    ...
    def isInMap(self, map, player, direction):
        ...
        return  (nextPosition[0] > Window.WIDTH // 2 + Display.BUFFER * Sprite.GROUND_TILE_SIZE) and \
                (nextPosition[0] < map.getSize()[0] - (Window.WIDTH // 2) - Display.BUFFER * Sprite.GROUND_TILE_SIZE) and \
                (nextPosition[1] > Window.HEIGHT // 2 + Display.BUFFER * Sprite.GROUND_TILE_SIZE) and \
                (nextPosition[1] < map.getSize()[1] - (Window.HEIGHT // 2) - Display.BUFFER * Sprite.GROUND_TILE_SIZE)